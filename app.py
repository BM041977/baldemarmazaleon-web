...
from contador import init_contador, incrementar_contador, ver_contador
...
# ════════════════════════════════════════════════════════════════
# CONTADOR DE VISITAS — baldemarmazaleon.com (landing page estática)
# ════════════════════════════════════════════════════════════════
# La landing page (sitio estático, sin backend propio) llama a estos
# 2 endpoints por JavaScript para mostrar cuántas visitas lleva.
# Empieza en 100. Los dispositivos de Baldemar quedan marcados en el
# NAVEGADOR (localStorage, ver landing_contador.js) y por eso llaman
# a /ver en vez de /incrementar — la exclusión no depende de la IP,
# que cambia según la red desde donde se conecte.
# Guardado en Turso (ver contador.py) en vez de un archivo local: el
# plan Free de Render borra los archivos locales en cada deploy, lo
# que reseteaba el contador a 100 con cada cambio subido al sitio.


@app.route("/api/contador/incrementar")
def api_contador_incrementar():
    return jsonify({"visitas": incrementar_contador()})


@app.route("/api/contador/ver")
def api_contador_ver():
    return jsonify({"visitas": ver_contador()})
...
def no_cache(response):
    ...
    # CORS solo para el contador: la landing page (baldemarmazaleon.com)
    # es un dominio distinto a este, y necesita poder llamar a estas
    # 2 rutas por JavaScript.
    if request.path.startswith("/api/contador/"):
